...
author: "Chaoran Hu, Jun Yan, Vladimir Pozdnyakov"
fontsize: 12pt
...
|                | fitMovResHun.seasonal | fitMovResHun; fitMovResHun.parallel |

#2. Overall analysis

In this section, we show how to play with `fitBmme`, `fitMovRes`, `fitMovResHun`
and `fitMovResHun.parallel`. All of these functions are designed for analysis
whole dataset. Note that the last function `fitMovResHun.parallel` is the parallel
computation wrapper for `fitMovResHun`, which inproves the speed of code significantly.
...
fitMovResHun(f109_std2, start = c(4, 0.4, 0.1, 1, 0.5))
fitMovResHun.parallel(f109_std2, start = c(4, 0.4, 0.1, 1, 0.5),
...
Now, we can apply `fitBmme.seasonal`, `fitMovRes.seasonal`, `fitMovResHun.seasonal` on
...
## Moving-resting-hunting process with embedded Brownian motion
fitMovResHun.seasonal(f109_summer, start = c(4, 0.4, 0.1, 1, 0.5),
                      ## number of threads allocated for parallel computation
                      numThreads = 6)
```
...
`rbmme`, `rMovRes`, and `rMovResHun`. The simulation result is based on a given time
gird.
...
simul_movres <- rMovRes(tgrid, lamM = 4, lamR = 0.4,
                        sigma = 1, s0 = "m")
simul_movreshun <- rMovResHun(tgrid, lamM = 4, lamR = 0.4, lamH = 0.1,
                              sigma = 1, p = 0.8, s0 = "m")

head(simul_bmme)
head(simul_movres)
head(simul_movreshun)
```
